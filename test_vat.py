...
def test_calculate_result(capsys):
    # case nothing found
    test_vat = vat.VAT("test URL")
    test_vat.calculate_result()
    assert capsys.readouterr().out == "Nothing found\n"

    # case successful test
    test_vat = vat.VAT("test URL")
    test_vat.correct = {"doc_language":1, "alt_texts":0, "input_labels":1, "empty_buttons":0, "empty_links":1, "color_contrast":0}
    test_vat.wrong = {"doc_language":0, "alt_texts":1, "input_labels":0, "empty_buttons":1, "empty_links":0, "color_contrast":1}
    test_vat.calculate_result()
