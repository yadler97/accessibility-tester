...
def check_input_label():
    ...
    for input_tag in input_tags:
        ...
        if "type" in input_tag.attrs and not input_tag['type'] == "hidden" and not input_tag['type'] == "submit":
...
def get_css_attribute_value(text, attribute):
    ...
    # TODO IDs have the second highest specificity
    if "id" in text.attrs:
        for sheet in stylesheets:
            rules = get_rules(sheet, class_list)
            for rule in rules:
                if not rule.style[attribute] == "":
                    return rule.style[attribute]

    # classes have the third highest specificity
    ...
    # html tags have the least highest specificity
    tag_name = text.name
    for sheet in stylesheets:
        rules = get_rules(sheet, class_list)
        for rule in rules:
...
def get_background_color_attribute(text):
    ...
    elif background_color == None and "type" in text.attrs and text["type"] == "submit":
